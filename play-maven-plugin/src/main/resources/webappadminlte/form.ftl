...
<section class="content-header">
    <h1>
    ${functionShowName}列表
        <small>开始管理${functionShowName}</small>
    </h1>
    <ol class="breadcrumb">
        <li><a href="#page/center/home"><i class="fa fa-home"></i> Home</a></li>
        <li><a href="#page/${moduleName}/${'${shortSimpleName}'?uncap_first}/${'${shortSimpleName}'?uncap_first}List"><i
                class="glyphicon glyphicon-list"></i> ${functionShowName}列表</a></li>
        <li class="active"
            th:text="${r"${"}${'${shortSimpleName}'?uncap_first}==null?'新建${functionShowName}':'编辑${functionShowName}'}">
            新建${functionShowName}</li>
    </ol>
</section>
<!-- Main content -->
        ...
        <div class="col-xs-12">
            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title"
                        th:text="${r"${"}${'${shortSimpleName}'?uncap_first}==null?'新建${functionShowName}':'编辑${functionShowName}'}">
                        新建${functionShowName}</h3>
                </div>
                <!-- /.box-header -->
                <!-- form start -->
                <form role="form" class="${'${shortSimpleName}'?lower_case}-form"
                      action="/${moduleName}/${'${shortSimpleName}'?uncap_first}/save">
                    <input type="hidden" name="id"
                           th:value="${r"${"}${'${shortSimpleName}'?uncap_first}==null?'':${'${shortSimpleName}'?uncap_first}.id}"/>
                    <div class="box-body">
                    <#list fields as field>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group date" id="${field.fieldName}picker">
                                    ...
                                    <input type="text" class="form-control pull-right"
                                           name="${field.fieldName}" <#if field.notnull?exists&&field.notnull >
                                           data-bv-notempty="true"
                                           data-bv-notempty-message="请填写${field.fieldName}"</#if>
                                           th:value="${r"${"}${'${shortSimpleName}'?uncap_first}==null||${'${shortSimpleName}'?uncap_first}.${field.fieldName}==null?'': #dates.format(${'${shortSimpleName}'?uncap_first}.${field.fieldName}, 'yyyy年MM月dd日 HH:mm')}"/>
                                </div>
                            </div>
                        <#elseif field.formtype="enum">
                            <div class="form-group">
                                <label for="${field.fieldName}Input">${field.fieldShowName}</label>
                                <select id="${field.fieldName}Input" name="type"
                                        class="form-control" <#if field.notnull?exists&&field.notnull >
                                        data-bv-notempty="true" data-bv-notempty-message="请选择${field.fieldName}"</#if>
                        ...
                        <#elseif field.formtype="textarea">
                            <div class="form-group">
                                <label for="${field.fieldName}Input">${field.fieldShowName}</label>
                                <textarea type="text" name="${field.fieldName}" class="form-control"
                                          id="${field.fieldName}Input" rows="3" placeholder="请填写${field.fieldName}"
                                    <#if field.notnull?exists&&field.notnull > data-bv-notempty="true"
                                          data-bv-notempty-message="请填写${field.fieldName}"</#if><#if field.minSize?exists>data-bv-stringlength="true"
                                          data-bv-stringlength-min="${field.minSize}"
                                          data-bv-stringlength-max="${field.maxSize}"
                                          data-bv-stringlength-message="个数必须在${field.minSize}和${field.maxSize}之间"
                                          <#elseif field.size?exists>data-bv-stringlength="true"
                                          data-bv-stringlength-min="0" data-bv-stringlength-max="${field.size}"
                                          data-bv-stringlength-message="个数必须在0和${field.size}之间"</#if>
                                          th:text="${r"${"}${'${shortSimpleName}'?uncap_first}==null?'':${'${shortSimpleName}'?uncap_first}.${field.fieldName}}"/>
                            </div>
                        <#else>
                            <div class="form-group">
                                <label for="${field.fieldName}Input">${field.fieldShowName!''}</label>
                                <input type="text" name="${field.fieldName}" class="form-control"
                                       id="${field.fieldName}Input" placeholder="请填写${field.fieldName}"
                                    <#if field.notnull?exists&&field.notnull > data-bv-notempty="true"
                                       data-bv-notempty-message="请填写${field.fieldName}"</#if><#if field.minSize?exists>data-bv-stringlength="true"
                                       data-bv-stringlength-min="${field.minSize}"
                                       data-bv-stringlength-max="${field.maxSize}"
                    ...
                    <div class="box-footer">
                        <button type="button" class="btn btn-default"
                                onclick="location.href='#/page/${moduleName}/${'${shortSimpleName}'?uncap_first}/${'${shortSimpleName}'?uncap_first}List'">
                            返回列表
                        </button>
